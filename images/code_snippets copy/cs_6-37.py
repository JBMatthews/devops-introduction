...
def main(): 
    module = AnsibleModule(
          ...
          server = xmlrpclib.ServerProxy('%s/xmlrpc.php' % params['url'], use_ datetime=True) 
...
for k,v in details.iteritems():
  if current_user[k] != details[k]: 
     ...
     is_changed = True 
  if is_changed and not module.check_mode:     server.wp.editProfile(1, params['username'], params['password'], details) 
